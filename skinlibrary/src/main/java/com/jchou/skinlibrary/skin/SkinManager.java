...
import com.jchou.skinlibrary.skin.listener.ISkinLoader;
...
public class SkinManager implements ISkinLoader {
    ...
    public int getColorStatusBar() {
        if (mResources != null) {
            //需要在资源文件里添加 colorStatusBar的color资源
            int identify = mResources.getIdentifier("colorStatusBar", "color", skinPackageName);
            if (identify>0){
                return mResources.getColor(identify);
            }else {
                return Color.WHITE;//否则默认返回白色
            }
        }
        return -1;
    }
